...
import threading
import time
from resources.lib.passlib.exc import PasslibHashWarning, PasslibConfigWarning
from resources.lib.passlib.utils.compat import PY3, JYTHON
import warnings
from warnings import warn
# site
# pkg
from resources.lib.passlib import exc
from resources.lib.passlib.exc import MissingBackendError
import resources.lib.passlib.registry as registry
from resources.lib.passlib.tests.backports import TestCase as _TestCase, skip, skipIf, skipUnless, SkipTest
from resources.lib.passlib.utils import has_rounds_info, has_salt_info, rounds_cost_values, \
                          rng as sys_rng, getrandstr, is_ascii_safe, to_native_str, \
                          repeat_string, tick, batch
from resources.lib.passlib.utils.compat import iteritems, irange, u, unicode, PY2, nullcontext
...
class TestCase(_TestCase):
    ...
    * suite of methods for matching against warnings
    ...
    def require_stringprep(self):
        ...
        from resources.lib.passlib.utils import stringprep
        if not stringprep:
            from resources.lib.passlib.utils import _stringprep_missing_reason
...
class HandlerCase(TestCase):
    """base class for testing password hash handlers (esp resources.lib.passlib.utils.handlers subclasses)
    ...
    def setUp(self):
        ...
        from resources.lib.passlib.utils import handlers
    ...
    def prepare_salt(self, salt):
        ...
        if self.fuzz_salts_need_bcrypt_repair:
            from resources.lib.passlib.utils.binary import bcrypt64
    ...
    def test_63_large_secret(self):
        ...
        from resources.lib.passlib.exc import PasswordSizeError
        from resources.lib.passlib.utils import MAX_PASSWORD_SIZE
    ...
    def test_64_forbidden_chars(self):
        ...
        if isinstance(chars, bytes):
            from resources.lib.passlib.utils.compat import iter_byte_chars
    ...
    def assert_is_masked(self, value):
        """
        check value properly masked by :func:`resources.lib.passlib.utils.mask_value`
        """
        if value is None:
            return
        self.assertIsInstance(value, unicode)
    ...
    def test_77_fuzz_input(self, threaded=False):
        ...
        * runs output against other "external" verifiers such as OS crypt()
        ...
        # gather info
        from resources.lib.passlib.utils import tick
...
class OsCryptMixin(HandlerCase):
    ...
    * check that native crypt support is detected correctly for known platforms.
    ...
    def _get_skip_backend_reason(cls, backend):
        ...
        from resources.lib.passlib.utils import has_crypt
    ...
    def _use_mock_crypt(self):
        """
        patch resources.lib.passlib.utils.safe_crypt() so it returns mock value for duration of test.
    ...
    def test_81_crypt_fallback(self):
        ...
        if self.has_os_crypt_fallback:
            ...
            from resources.lib.passlib.exc import InternalBackendError as err_type
            hash = self.get_sample_hash()[1]
    ...
    def fuzz_verifier_crypt(self):
        ...
        from resources.lib.passlib.utils import _safe_crypt_lock
