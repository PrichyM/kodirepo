"""resources.lib.passlib.handler - code for implementing handlers, and global registry for handlers"""
#=============================================================================
# imports
#=============================================================================
...
# site
# pkg
import resources.lib.passlib.exc as exc, resources.lib.passlib.ifc as ifc
from resources.lib.passlib.exc import MissingBackendError, PasslibConfigWarning, \
                        PasslibHashWarning
from resources.lib.passlib.ifc import PasswordHash
from resources.lib.passlib.registry import get_crypt_handler
...
from resources.lib.passlib.utils.binary import (
    BASE64_CHARS, HASH64_CHARS, PADDED_BASE64_CHARS,
    HEX_CHARS, UPPER_HEX_CHARS, LOWER_HEX_CHARS,
    ALL_BYTE_VALUES,
)
from resources.lib.passlib.utils.compat import join_byte_values, irange, u, native_string_types, \
                                 uascii_to_str, join_unicode, unicode, str_to_uascii, \
                                 join_unicode, unicode_or_bytes_types, PY2, int_types
from resources.lib.passlib.utils.decor import classproperty, deprecated_method
# local
__all__ = [
...
def guess_app_stacklevel(start=1):
    ...
    looks for first frame not part of resources.lib.passlib.
    """
    ...
    try:
        while frame:
            ...
            if name.startswith("resources.lib.passlib.tests.") or not name.startswith("resources.lib.passlib."):
                return max(1, count)
            count += 1
            frame = frame.f_back
        return start
...
class HasManyIdents(GenericHandler):
    ...
    def using(cls,  # keyword only...
        ...
        """
        This mixin adds support for the following :meth:`~resources.lib.passlib.ifc.PasswordHash.using` keywords:
...
class BackendMixin(PasswordHash):
    ...
    .. classmethod:: _load_backend_{name}(dryrun=False)
        ...
        :raises resources.lib.passlib.exc.PasslibSecurityError:
    ...
    def get_backend(cls):
        ...
        :raises resources.lib.passlib.exc.MissingBackendError:
...
class PrefixWrapper(object):
    ...
    this is used mainly by the :doc:`ldap crypt <resources.lib.passlib.hash.ldap_crypt>` handlers;
    such as :class:`~resources.lib.passlib.hash.ldap_md5_crypt` which wraps :class:`~resources.lib.passlib.hash.md5_crypt` and adds a ``{CRYPT}`` prefix.
