"""resources.lib.passlib.crypto.digest -- crytographic helpers used by the password hashes in passlib
...
from resources.lib.passlib.utils import join_bytes, to_native_str, join_byte_values, to_bytes, \
                          SequenceMixin, as_bool
from resources.lib.passlib.utils.compat import irange, int_types, unicode_or_bytes_types, PY3, error_from
...
def _get_hash_aliases(name):
    ...
    if name.startswith("scram-"): # helper for SCRAM protocol (see resources.lib.passlib.handlers.scram)
        name = name[6:]
...
def _get_hash_const(name):
    ...
    try:
        ...
        const.__doc__ = ("wrapper for hashlib.new(%r),\n"
                         "generated by resources.lib.passlib.crypto.digest.lookup_hash()") % name
        return const

    # use builtin md4 as fallback when not supported by hashlib
    if name == "md4":
        from resources.lib.passlib.crypto._md4 import md4
        return md4
...
def lookup_hash(digest,  # *,
    ...
    :param required:
        By default (True), this function will throw an :exc:`~resources.lib.passlib.exc.UnknownHashError` if no hash constructor
...
def pbkdf2_hmac(digest, secret, salt, rounds, keylen=None):
    ...
    .. versionchanged:: 1.7
        ...
        * builtin pure-python backend
        ...
        See :data:`resources.lib.passlib.crypto.digest.PBKDF2_BACKENDS` to determine
...
if PY3 and _force_backend in ["any", "from-bytes"]:
    ...
    from struct import Struct
    from resources.lib.passlib.utils import sys_bits
    ...
    def _get_pbkdf2_looper(digest_size):
        ...
        # compile helper
        #
        code = compile(source, "<generated by resources.lib.passlib.crypto.digest._get_pbkdf2_looper()>", "exec")
    ...
    from resources.lib.passlib.utils import int_to_bytes
